...
.PHONY: build-push-all gpu-build-push-all
...
gpu-build-image:
	@echo "Building web Docker image: $(IMAGE_NAME)-gpu:$(IMAGE_VERISON)..."
	docker buildx build --platform $(PLATFORMS) -t $(IMAGE_NAME)-gpu:$(IMAGE_VERISON) -f Dockerfile.gpu ./ || $(call check_error,"build-image")
	@echo "Web Docker image built successfully: $(IMAGE_NAME)-gpu:$(IMAGE_VERISON)"

gpu-tag-image: 
	@echo "Taging web Docker image: $(IMAGE_NAME)-gpu:$(IMAGE_VERISON)..."
	docker tag $(IMAGE_NAME):$(IMAGE_VERISON) $(REGISTRY_URL)/$(IMAGE_NAME)-gpu:$(IMAGE_VERISON) || $(call check_error,"tag-image")
	@echo "Web Docker image tag successfully: $(IMAGE_NAME)-gpu:$(IMAGE_VERISON)"

gpu-push-image: 
	@echo "Pushing web Docker image: $(IMAGE_NAME):$(IMAGE_VERISON)..."
	docker push $(REGISTRY_URL)/$(IMAGE_NAME)-gpu:$(IMAGE_VERISON) || $(call check_error,"push-image")
	@echo "Web Docker image push successfully: $(IMAGE_NAME)-gpu:$(IMAGE_VERISON)"
...
gpu-run-image:
	...
	docker rm $(CONTAINER_NAME) > /dev/null 2>&1 || true
	@echo "Running web Docker container: $(CONTAINER_NAME)..."
	docker run -d --name $(CONTAINER_NAME) -p 8080:80 $(IMAGE_NAME)-gpu:$(IMAGE_VERISON) || $(call check_error,"run-image")
	@echo "Web Docker container running successfully: $(CONTAINER_NAME)"
...
gpu-clean-image:
	...
	docker stop $(CONTAINER_NAME) > /dev/null 2>&1 || true
	docker rm $(CONTAINER_NAME) > /dev/null 2>&1 || true
	docker rmi -f $(IMAGE_NAME)-gpu:$(IMAGE_VERISON) $(REGISTRY_URL)/$(IMAGE_NAME)-gpu:$(IMAGE_VERISON) > /dev/null 2>&1 || true
	# 清理未被任何容器引用的镜像
	docker image prune -f -a > /dev/null 2>&1 || true
	@echo "Web Docker container and image cleaned successfully."

gpu-build-push-all: gpu-build-image gpu-tag-image gpu-push-image
